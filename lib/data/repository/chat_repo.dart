...
class ChatRepo {
  ...
  Future<http.StreamedResponse> sendMessage(
    ...
    http.MultipartRequest request = http.MultipartRequest('POST',
        Uri.parse('${AppConstants.BASE_URL}${AppConstants.GET_IMAGES_URL}'));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    if (images != null && ResponsiveHelper.isMobilePhone()) {
      ...
      for (int i = 0; i < images.length; i++) {
        ...
        request.files.add(http.MultipartFile(
            'image[]', images[i].readAsBytes().asStream(), _list.length,
  ...
  Future<ResponseModel> sendRealTimeMessage(int senderID, UserType senderType,
      int receiverID, UserType receiverType, String message, List<XFile> images,
      {int orderID}) async {
